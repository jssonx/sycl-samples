...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    if (gpu_devices.size() < 2) {
      std::cout << "Not enough GPU devices available. At least 2 GPUs are required.\n";
      return -1;
    }
    ...
    std::cout << "Using device 0: " << gpu_devices[0].get_info<sycl::info::device::name>() << "\n";
    std::cout << "Using device 1: " << gpu_devices[1].get_info<sycl::info::device::name>() << "\n";

    for (int i = 0; i < 2; ++i) {
      a_matrices[i] = static_cast<float(*)[N]>(sycl::malloc_shared(M * N * sizeof(float), queues[i]));
      b_matrices[i] = static_cast<float(*)[P]>(sycl::malloc_shared(N * P * sizeof(float), queues[i]));
      c_matrices[i] = static_cast<float(*)[P]>(sycl::malloc_shared(M * P * sizeof(float), queues[i]));

      if (!a_matrices[i] || !b_matrices[i] || !c_matrices[i]) {
        throw std::runtime_error("USM allocation failed for matrix set " + std::to_string(i));
      }
  ...
  for (int i = 0; i < 2; ++i) {
    verifyResult(c_matrices[i], full_verify);
  }

  ...
  if (run_tracker) {
    ...
    if (ret != 0 ) {
      fprintf(stderr, "ERROR -- launch of \"system(tracker)\" failed, returning %d; errno = %d, msg = %s\n", ret, errno, strerror(errno) );
       ...
       //fprintf(stderr, "tracker run done\n" );
    }
  }
}
...
mpitracker()
{
  ...
  if (run_mpitracker) {
    ...
    if (ret != 0 ) {
      ...
      fprintf(stderr, "mpitracker run done\n" );
    }
  }
}
...
setup_run(int argcc, char **argvv)
{
  ...
  thispid = getpid();
  fprintf(stderr, "    [%d] minitest process started\n", thispid );
  ...
  if (s != NULL) { 
    fprintf(stderr, "    [%d] Running of tracker enabled\n", thispid );
    run_tracker = true;
  } else {
    // fprintf(stderr, "    [%d] Running of tracker disabled\n", thispid );
    run_tracker = false;
  }
  ...
  if (s != NULL) { 
    fprintf(stderr, "    [%d] Running of mpitracker enabled\n", thispid );
    run_mpitracker = true;
  } else {
    // fprintf(stderr, "    [%d] Running of mpitracker disabled\n", thispid );
  ...
  if (rex != MPI_SUCCESS) {
    ...
    fprintf (stderr, "    [%d] MPI_Init succeeded, returning %d\n", thispid, rex);
  }
  ...
  if (res != MPI_SUCCESS) {
    ...
    // fprintf (stderr, "    [%d] MPI_Comm_rank = %d \n", thispid, mpi_rank);
  }
  // invoke mpitracker
   mpitracker();
#endif
  ...
  fprintf(stderr, "       [%d] overhead of real-time delta measurement: %10.3f ns.\n\n",
    thispid, tempus);
}
...
mpisync()
{
  ...
  if (res != MPI_SUCCESS) {
    ...
    // fprintf (stderr, "    [%d] MPI_Barrier succeeded for MPI Rank %d \n", thispid, mpi_rank);
  }
#endif
}
...
teardown_run(void)
{
  ...
  fprintf(stderr, "    [%d]   minitest app exiting at timestamp %13.9f s.\n",
    thispid, tempus );
  fprintf(stderr, "======================================================\n");
}
...
checkdata(int threadnum, double *p, size_t size)
{
  ...
  if (cnt != 0) {
    ...
    fprintf(stderr, "      [%d] all good -- thread %d:  good count = %d\n",
